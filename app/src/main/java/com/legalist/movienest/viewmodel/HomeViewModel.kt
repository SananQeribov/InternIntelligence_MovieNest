...
class HomeViewModel() : ViewModel() {
    private val _movieList = MutableLiveData<Resource<List<MovieItem?>>>()
    val movieList: LiveData<Resource<List<MovieItem?>>> get() = _movieList
    ...
    fun getMovieList() {
        viewModelScope.launch {
           // _movieList.postValue(Resource.Loading())  // show loading state
            ...
            try {
                val response = ApiClient.getClient().getMovieData(token = Constant.bear_token)
                ...
                val resource = when {
                    response.isSuccessful -> {
                       ...
                       _movieList.postValue(Resource.Success(movieItem))
                ...
                _movieList.postValue(Resource.Error("Exception: ${e.localizedMessage ?: "An unknown error occurred"}"))
            }
            finally {
